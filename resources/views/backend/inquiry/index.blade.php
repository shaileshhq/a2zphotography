					...
					<div class="card-header">
						<div class="row align-items-center">
							...
							<div class="col-lg-3 col-xl-2">
								<a href="{{ route('inquiry.create') }}" class="btn btn-primary radius-30 mt-2 mt-lg-0"><i class="bx bxs-plus-square"></i>Add Enquiry</a>
							</div>
						</div>
					</div>
					<div class="card-body">
						<div class="table-responsive">
							<table class="table mb-0">
								...
								@foreach ($inquiry as $item)
									<tr>
										<td>{{$i++}}</td>
										<td>@php $product =App\Models\Product::where('id',$item->product_id)->first() @endphp {{$product->title}}</td>
										<td>{{$item->name}}</td>
										<td>{{$item->phone}}</td>
										<td>{{$item->email}}</td>
										<td>{{$item->message}}</td>
										<td>
											 <div class="d-flex order-actions">
												<a href="{{route('inquiry.delete', $item->id)}}" class=""><i class='bx bxs-trash'></i></a>
											</div>
										</td>
									</tr>
