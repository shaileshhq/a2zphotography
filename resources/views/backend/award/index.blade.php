					...
					<div class="ps-3">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb mb-0 p-0">
								...
								<li class="breadcrumb-item active" aria-current="page">All Awards & Certificate</li>
							</ol>
						</nav>
					</div>
				</div>
					...
					<div class="card-header">
						<div class="row align-items-center">
							...
							<div class="col-lg-3 col-xl-2">
								<a href="{{ route('award.create') }}" class="btn btn-primary radius-30 mt-2 mt-lg-0"><i class="bx bxs-plus-square"></i>Add Award & Certificate</a>
							</div>
						</div>
					</div>
					<div class="card-body">
						<div class="table-responsive">
							<table class="table mb-0">
								...
								<tbody>
									...
									<tr>
										<td>{{$i++}}</td>
										<td><img src="../public/award/{{$item->image}}" style="width:150px;"></td>
										<td>{{$item->title}}</td>
										<td>{{$item->short_desc}}</td>
										<td>
											<div class="d-flex order-actions">
												<a href="{{ route('award.edit', $item->id) }}" class=""><i class='bx bxs-edit'></i></a>
												<a href="{{route('award.delete', $item->id)}}" class="ms-3"><i class='bx bxs-trash'></i></a>
											</div>
										</td>
									</tr>
									@endforeach
